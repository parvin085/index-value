...
const data = [
    ...
    { "name": "Bitcoin Cash", "ticker": "BCH", "value": "441.4", "change": "5.01%" }, {"name": "Bitcoin SV", "ticker": "BSV", "value": "303.17", "change": "5.53%"},
    ...
    { "name": "Tezos", "ticker": "XTZ", "value": "2.1247", "change": "6.12%" }, { "name": " Ethereum Classic", "ticker": "ETC", "value": "12.5988", "change": "4.09%"},
...
data.forEach(item => {
    let =  val = Number(item.change.slice(0, -1))
    tbodyElement.innerHTML += `
        <tr>
            <td>${item.name}</td>
            <td>${item.ticker}</td>
            <td>${item.value}</td>
            <td class="${val > 0 ?"green" : "red"}">${item.change}</td>
        </tr>`
